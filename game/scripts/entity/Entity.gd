...
func _build_stats() -> void:
	...
	if ResourceLoader.exists(script_path) and not def.is_empty():
		...
		if sc != null:
			stats = sc.new()
			if stats != null and stats.has_method("from_entity_def"):
				var d2 := def.duplicate()
				d2["stats"] = s
				stats.call("from_entity_def", d2)
				if "race_id" in stats:
					stats.set("race_id", String(def.get("race", "human")))
...
func ai_enabled() -> bool:
	return not dead and _dist_to_player <= AI_CULL_DIST and not Game.paused_by_ui
